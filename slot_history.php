...
<head>
    ...
    <title>Charging Completed - Electron</title>
</head>
...
<body>
    ...
    <div class="right_to_menu">
        <div class="table-slot-container">
            <?php
            $res = mysqli_query($conn, "SELECT a.*, b.*,c.* FROM user_form a JOIN slot_booking b On a.id = b.user_id JOIN host_schedule c On b.scheduleid = c.schedule_id Where hostid = '$host_id' && sch_status = 'Approved' order by booking_id DESC ");
            if (!$res) {
                printf("Error: %s\n", mysqli_error($conn));
                exit();
            }
