...
<head>
    ...
    <title>Host Details - Electron</title>
</head>
...
<body>
    ...
    <div class="table-slot-container">
        <?php
        $res = mysqli_query($conn, "SELECT a.*, b.*,c.* FROM user_form a JOIN slot_booking b On a.id = b.user_id JOIN host_schedule c On b.scheduleid = c.schedule_id Where hostid = '$host_id' && sch_status = 'Done' order by booking_id DESC ");
        if (!$res) {
            printf("Error: %s\n", mysqli_error($conn));
            exit();
        }
        ...
        while ($slot_result = mysqli_fetch_array($res)) {
            ...
            echo "<td>" . $slot_result['status'] . "</td>";
            echo "<form method='POST'>";
            echo "<td>";
            echo '<a href="charging_done.php?id=' . $slot_result['booking_id'] . '" class ="accept">END</a>';
            echo "</td>";                       
        }
        ?>
    </div>
