...
<head>
    ...
    <title>Pending Requests- Electron</title>
...
<body>
    ...
    <div class="admin_menu_row">
        ...
        <a href="suspended_host.php"><i class='fas fa-user-slash'></i> Suspended Host</a>
    </div>

    <div class="table_container">
        <?php
        require_once "config.php";
        $sql = "SELECT * FROM user_form WHERE user_type ='host' && Status = 'Pending' OR Status = '' ";
        if ($result = mysqli_query($conn, $sql)) {
            if (mysqli_num_rows($result) > 0) {
                ...
                while ($row = mysqli_fetch_array($result)) {
                    echo "<tr>";
                    echo "<td>" . $row['name'] . "</td>";
                    echo "<td> +91 - " . $row['phone_number'] . "</td>";
                    echo "<td>" . $row['email'] . "</td>";
                    echo "<td>";
                    echo '<a href="approve_request.php?id=' . $row['id'] . '" class="accept" >Approve</a>';
                    echo "</td>";
                    echo "<td>";
                    echo '<a href="decline_request.php?id=' . $row['id'] . '" class="reject">Decline</a>';
                    echo "</td>";
                    echo "</tr>";
                }
                echo "</tbody>";
